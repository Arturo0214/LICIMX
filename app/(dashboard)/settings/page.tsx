...
interface TeamMember {
  id: string; name: string; email: string; role: string; roleLabel: string; initials: string; isActive: boolean
}
...
interface Integration {
  id: string; name: string; description: string; status: "conectado" | "desconectado" | "configurando"; icon: string
}
...
export default function SettingsPage() {
  const { org, notifPrefs, saveOrg, toggleNotif } = useSettings()

  const [orgName, setOrgName] = useState(org.name)
  const [orgRfc, setOrgRfc] = useState(org.rfc)
  const [orgIndustry, setOrgIndustry] = useState(org.industry)
  const [orgSaved, setOrgSaved] = useState(false)
  const [notifSaved, setNotifSaved] = useState(false)

  // Sync when org changes from hook
  useState(() => {
    setOrgName(org.name)
    setOrgRfc(org.rfc)
  ...
  const handleSaveOrg = () => {
    saveOrg({ name: orgName, rfc: orgRfc, industry: orgIndustry })
    setOrgSaved(true)
    setTimeout(() => setOrgSaved(false), 2000)
  }

  const handleSaveNotif = () => {
    // Already persisted on each toggle, but show feedback
    setNotifSaved(true)
  ...
  return (
    <div className="space-y-8">
      ...
      <Tabs defaultValue="organizacion">
        <TabsList>
          <TabsTrigger value="organizacion"><Building2 className="mr-2 h-4 w-4" />Organizacion</TabsTrigger>
          <TabsTrigger value="usuarios"><Users className="mr-2 h-4 w-4" />Usuarios</TabsTrigger>
          <TabsTrigger value="scoring"><Target className="mr-2 h-4 w-4" />Scoring</TabsTrigger>
          <TabsTrigger value="notificaciones"><Bell className="mr-2 h-4 w-4" />Notificaciones</TabsTrigger>
          <TabsTrigger value="integraciones"><Plug className="mr-2 h-4 w-4" />Integraciones</TabsTrigger>
        </TabsList>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <Button variant="outline" size="sm" className="mt-2 text-xs">Subir imagen</Button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={orgRfc} onChange={(e) => setOrgRfc(e.target.value)} className="font-mono" />
                </div>
                <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {orgSaved && <span className="text-sm text-success flex items-center gap-1"><CheckCircle2 className="h-4 w-4" /> Guardado</span>}
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="usuarios" className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm"><UserPlus className="mr-2 h-3.5 w-3.5" />Invitar</Button>
          </div>
              ...
              <TableBody>
                {teamMembers.map((member) => (
                  <TableRow key={member.id} className="border-border/50">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar className="h-8 w-8"><AvatarFallback className="bg-muted text-xs text-muted-foreground">{member.initials}</AvatarFallback></Avatar>
                        <span className="text-sm font-medium text-foreground">{member.name}</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">{member.email}</TableCell>
                    <TableCell><Badge className={cn("border text-[10px]", roleColors[member.role])}>{member.roleLabel}</Badge></TableCell>
                    <TableCell>
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <Button variant="outline" size="sm" className="mt-4" asChild>
                <a href="/scoring"><ExternalLink className="mr-2 h-3.5 w-3.5" />Ir a Scoring</a>
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="notificaciones" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Selecciona como quieres recibir cada tipo de alerta.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="rounded-lg border border-border">
                <div className="grid grid-cols-4 gap-4 border-b border-border px-4 py-3">
                  ...
                  <div className="flex items-center justify-center gap-1 text-xs font-medium text-muted-foreground"><Mail className="h-3.5 w-3.5" />Email</div>
                  <div className="flex items-center justify-center gap-1 text-xs font-medium text-muted-foreground"><MessageSquare className="h-3.5 w-3.5" />WhatsApp</div>
                  <div className="flex items-center justify-center gap-1 text-xs font-medium text-muted-foreground"><Smartphone className="h-3.5 w-3.5" />In-App</div>
                </div>
                {notifPrefs.map((pref, index) => (
                  <div
                    key={pref.event}
                    className={cn("grid grid-cols-4 gap-4 px-4 py-3 transition-colors hover:bg-muted/30", index < notifPrefs.length - 1 && "border-b border-border/50")}
                  >
                    <div className="text-sm text-foreground">{pref.event}</div>
                    <div className="flex justify-center"><Checkbox checked={pref.email} onCheckedChange={() => toggleNotif(index, "email")} /></div>
                    <div className="flex justify-center"><Checkbox checked={pref.whatsapp} onCheckedChange={() => toggleNotif(index, "whatsapp")} /></div>
                    <div className="flex justify-center"><Checkbox checked={pref.in_app} onCheckedChange={() => toggleNotif(index, "in_app")} /></div>
                  </div>
                ))}
              </div>
              <div className="mt-4 flex items-center gap-3">
                ...
                {notifSaved && <span className="text-sm text-success flex items-center gap-1"><CheckCircle2 className="h-4 w-4" /> Guardado</span>}
              </div>
            </CardContent>
          </Card>
        </TabsContent>
                ...
                <Card key={integration.id}>
                  <CardContent className="p-5">
                    <div className="flex items-start gap-4">
                      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-muted text-sm font-bold text-foreground">{integration.icon}</div>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-3 flex items-center gap-3">
                          ...
                          <Button variant="outline" size="sm" className="h-7 text-xs px-2">Configurar</Button>
                        </div>
                      </div>
                    </div>
                  </CardContent>
